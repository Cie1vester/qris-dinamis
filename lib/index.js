...
function extractAmountFromTag54(qris) {
  const tagIndex = qris.indexOf("54");
  if (tagIndex === -1) return null;
  const len = parseInt(qris.slice(tagIndex + 2, tagIndex + 4), 10);
  return qris.slice(tagIndex + 4, tagIndex + 4 + len);
}
...
function dataQris(qris) {
  ...
  const amount = extractAmountFromTag54(qris);

  const crcInput = qris.slice(0, -4); // data before CRC
  const crcFromQris = qris.slice(-4); // last 4 chars
  const crcComputed = toCRC16(crcInput); // correct CRC value

  console.log("CRC from QR:", crcFromQris);
  console.log("CRC computed :", crcComputed);
